...
angular.module("bahmni.registration").controller("CreatePatientController", [
  "$window",
  "$cookies",
  "$timeout",
  "$http",
  ...
  function (
    ...
    $scope.actions = {};
    var errorMessage;
    var configValueForEnterId = appService
      .getAppDescriptor()
      .getConfigValue("showEnterID");
    $scope.addressHierarchyConfigs = appService
      .getAppDescriptor()
      .getConfigValue("addressHierarchy");
    $scope.disablePhotoCapture = appService
      .getAppDescriptor()
      .getConfigValue("disablePhotoCapture");
    $scope.showEnterID =
      configValueForEnterId === null ? true : configValueForEnterId;
    $scope.today = Bahmni.Common.Util.DateTimeFormatter.getDateWithoutTime(
      dateUtil.now()
    );
    ...
    var countRegistration = function (userUuid) {
      let apiUrl =
        "/openmrs/module/bahmnicustomutil/countRegistrationByUser/" +
        userUuid +
        ".form";
      $http({
        method: "GET",
    ...
    var prepopulateDefaultsInFields = function () {
      ...
      var patientInformation = appService
        .getAppDescriptor()
        .getConfigValue("patientInformation");
      ...
      var setDefaultAnswer = function (personAttributeType) {
        $scope.patient[personAttributeType.name] =
          defaults[personAttributeType.name];
      };
      ...
      var setDefaultConcept = function (personAttributeType) {
        ...
        _.chain(personAttributeType.answers)
          .filter(isDefaultAnswer)
    ...
    var expandSectionsWithDefaultValue = function () {
      angular.forEach(
        $rootScope.patientConfiguration &&
          $rootScope.patientConfiguration.getPatientAttributesSections(),
        function (section) {
          var notNullAttribute = _.find(
          ...
          section.expand =
            section.expanded || (notNullAttribute ? true : false);
        }
      );
    };
    ...
    var prepopulateFields = function () {
      var fieldsToPopulate = appService
        .getAppDescriptor()
        .getConfigValue("prepopulateFields");
      if (fieldsToPopulate) {
        _.each(fieldsToPopulate, function (field) {
          ...
          if (addressLevel) {
            $scope.patient.address[addressLevel.addressField] =
              $rootScope.loggedInLocation[addressLevel.addressField];
          }
        });
      }
    };
    ...
    var addNewRelationships = function () {
      var newRelationships = _.filter(
        $scope.patient.newlyAddedRelationships,
        function (relationship) {
          return (
            relationship.relationshipType && relationship.relationshipType.uuid
          );
        }
      );
