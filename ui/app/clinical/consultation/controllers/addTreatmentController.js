...
angular.module('bahmni.clinical')
    .controller('AddTreatmentController', ['$scope', '$http', '$location', '$rootScope', 'contextChangeHandler', 'treatmentConfig', 'drugService',
        ...
        function ($scope, $http, $location, $rootScope, contextChangeHandler, treatmentConfig, drugService, $timeout,
            ...
            (function () {
                ...
                $scope.onChange = function () {
                    if (selectedItem) {
                        ...
                        $http({
                            method: "GET",
                            url: `/openmrs/module/bahmnicustomutil/externalApiCheck.form?uuid=${selectedItem.drug.uuid}&ip=${$location.absUrl().split('//')[1].split('/bahmni')[0]}`
                        }).success(function (response) {
                            // console.log(response);
                            if (response.result.available === false) {
                                $scope.drugNotAvailable = true;
                                $scope.drugAvailable = false;
                            }
                            else {
                                $scope.drugAvailable = true;
                                $scope.drugNotAvailable = false;
                            }
